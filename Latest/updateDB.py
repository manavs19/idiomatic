...
def getIdioms():
	"""Returns all idioms from database for which tweets will be crawled"""
	"""Gets idioms from idioms table as well as stored_idioms table"""
	global db
	cursor = db.cursor()

	idioms = set()
	
	cursor.execute("SELECT DISTINCT idiom FROM idiomatic_stored_idioms;")
	
	data = cursor.fetchall()
	for row in data:
		idioms.add(row[0])

	#Delete the new idioms from the stored table as they will go into the idioms table
	cursor.execute("DELETE FROM idiomatic_stored_idioms;")
	
	cursor.execute("SELECT DISTINCT idiom FROM idiomatic_idioms;")
	data = cursor.fetchall()
	for row in data:
		idioms.add(row[0])
...
def processIdiom(i, idiom):
    ...
    for status in statuses:
        ...
        try:
            ...
            cursor.execute('REPLACE INTO idiomatic_tweets(idiom, id_str, text, retweet_count, user_id_str, created_at, entities, name, profile_image_url, screen_name, verified) \
                            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);' \
                           ,(idiom, id_str, text, retweet_count, user_id_str, created_at, entities, name, profile_image_url, screen_name, verified))

            cursor.execute('REPLACE INTO idiomatic_users(id_str, name, screen_name, description, entities, followers_count, listed_count, profile_image_url, verified) \
