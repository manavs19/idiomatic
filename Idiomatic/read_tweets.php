...
function echoTweet($tweet)
{
	...
	if (array_key_exists('media', $encoded_entities)) {
	    ...
	    foreach ($media as $key => $url) {
	    	$search = $url['url'];
	    	$replace = "<img src='$search' alt='Image not available!'></img>";//<a href='$search' rel='external'>$search</a>";
	    	$text = str_replace($search, $replace, $text);
	    }
	}
    $text = $text."</p>";
...
$query = "SELECT * FROM tweets WHERE `idiom` = '$idiom' ORDER BY retweet_count DESC ;";//LIMIT 50;";
if($query_run = mysql_query($query))
{
	if(mysql_num_rows($query_run)!=0)
	{
		...
		while ($tweet = mysql_fetch_assoc($query_run))
		{
			$text = $tweet["text"];
            ...
            foreach ($unique_tweets as $value) {
                if(getSimilarityCoefficient($text, $value, " ") > 0.49)
                {
                    $flag = 1;
                    break;
                }
